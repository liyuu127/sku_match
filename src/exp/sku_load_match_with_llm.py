...
5. 商品信息整体语义冲突，origin_product商品信息为小型便携式冲牙器，candidate商品信息为家用取暖器暖风机

请使用以下键值以有效的 JSON 格式进行响应,不要带有额外回车或json标识符：
...
async def main():
    ...
    top1_list, top2_list, top3_list = owner_subset['相似商品1（ID-名称）'], owner_subset['相似商品2（ID-名称）'], \
        owner_subset['相似商品3（ID-名称）']
    # 创建任务列表
    tasks = [
